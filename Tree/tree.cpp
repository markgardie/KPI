...
void Tree::show_rec(Node* root, int depth, char dir) {
    ...
    cout << dir << root->value << col << endl;

    if (root->left != NULL) show_rec(root->left, depth + 2, '-');
    else return;

    if (root->right != NULL) show_rec(root->right, depth + 2, '+');
    else return;
...
void Tree::insert(int value) {
    ...
    if (root == NULL) {insert_root(n); return;}
    else {insert_node(n, root); balance_insert(n);}
}
...
void Tree::insert_node(Node* n, Node* root) {
    if (n->value < root->value && root->left == NULL) { root->left = n; n->parent = root; n->left = NULL; n->right = NULL; return; }
    if (n->value > root->value && root->right == NULL) { root->right = n; n->parent = root; n->left = NULL; n->right = NULL; return; }
...
void Tree::balance_insert(Node* n) {
    if (n->uncle() != NULL) {
        if (n->parent->color && n->uncle()->color) repainting(n);
        if (n->parent->color && !n->uncle()->color) line(n);
    }
    else {
        line(n);
    }
...
void Tree::repainting(Node* n) {
    ...
    n->uncle()->color = false;
    if (n->grandparent() == root) n->grandparent()->color = false;
    else n->grandparent()->color = true;
...
void Tree::remove(int value) {
    ...
    delete old_node;
    nodes_count--;
    if(new_node->color && new_node->parent->color) balance_insert(new_node);
}
